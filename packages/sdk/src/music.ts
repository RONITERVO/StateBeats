import { nextRandom } from '@statebeats/core';
import { z } from 'zod';
import { compile } from './compiler.js';
import { EngineError, idSchema, musicChannels, musicSchema, parsed } from './schema.js';
import type { MapDefinition, MapInput, MusicFeatures, MusicTimeline } from './schema.js';
...
export const musicGenerationSchema = z
  .object({
    ...
    difficulty: z.enum(['gentle', 'flow', 'busy']).optional(),
    turning: z.boolean().optional(),
    theme: idSchema.optional(),
  })
  .strict();
/** A reproducible, reach-bounded chart on an explicitly supplied beat grid. No BPM guessing. */
export function generateMusicMap(
  ...
  options = parsed(musicGenerationSchema, options);
  const music = parsed(musicSchema, input),
    tickRate = options.tickRate ?? music.tickRate,
    bpm = options.bpm ?? 120;
  const difficulty = options.difficulty ?? 'gentle',
    originalSeed = options.seed ?? 1;
  if (
    !music.frames.length ||
    music.frames.some((frame, i) => i > 0 && frame.tick <= music.frames[i - 1].tick) ||
    ![60, 120, 240].includes(tickRate) ||
    tickRate !== music.tickRate ||
    !Number.isFinite(bpm) ||
    bpm < 40 ||
    bpm > 240 ||
    !['gentle', 'flow', 'busy'].includes(difficulty) ||
    !Number.isInteger(originalSeed) ||
    originalSeed < 1 ||
    originalSeed > 4294967295
  )
    throw new EngineError('VALIDATION', 'Invalid music timeline, seed, tempo or difficulty');
  const step = difficulty === 'gentle' ? 4 : difficulty === 'flow' ? 2 : 1;
  const beatTicks = (tickRate * 60) / bpm,
    endTick = music.frames[music.frames.length - 1].tick;
  const firstBeat = Math.max(4, Math.ceil(music.frames[0].tick / beatTicks) + 2),
    lastBeat = Math.floor(endTick / beatTicks) - 1;
  if (lastBeat < firstBeat)
    throw new EngineError(
      'MUSIC_TOO_SHORT',
      'Choose audio long enough for an approach and a playable phrase',
    );
  const map: MapInput = {
    version: 1,
    id: options.id ?? `music-${originalSeed}`,
    title: options.title ?? 'Your sunlit journey',
    description:
      'Locally generated from stored music features. Follow the sun; reach for stars. The saved chart is the gameplay authority.',
    seed: originalSeed,
    tickRate,
    durationBeats: endTick / beatTicks + 2,
    tempo: [{ beat: 0, bpm }],
    music,
    scene: {
      version: 1,
      theme: options.theme ?? 'statebeats/landscape',
      label: 'A traveling sun over water and distant mountains',
      objects: [
        {
          id: 'sun',
          ...
          color: '#ffd58a',
          react: [
            { channel: 'bass', property: 'scale', amount: 0.2 },
  ...
  const path = map.scene!.objects![0].motion!;
  // At most 129 path keys, even for long music. A gentle complete turn every 32 beats.
  const pathStep = Math.max(4, Math.ceil((lastBeat + 4) / 128));
  for (let beat = 0; beat <= lastBeat + pathStep; beat += pathStep)
    path.push({
      beat,
      position: {
        azimuth: options.turning ? (beat / 32) * 360 : -30 + (60 * beat) / Math.max(1, lastBeat),
        elevation: 20,
        radius: 18,
        height: 1.4,
      },
  ...
  let seed = originalSeed,
    index = 0;
  for (let beat = firstBeat; beat <= lastBeat; beat += step) {
    const features = sampleMusic(music, Math.round(beat * beatTicks));
    if (features.rms < 0.004) continue;
    const random = nextRandom(seed);
    seed = random.state;
    const semantic = index % 2 ? 'right' : 'left',
      direction = options.turning
        ? ((beat - 2) / 32) * 360
        : -30 + (60 * (beat - 2)) / Math.max(1, lastBeat);
    map.notes.push({
      id: `star-${index}`,
      beat,
      preset: semantic,
      ...
      label: `${semantic} star`,
      position: {
        azimuth: direction + (semantic === 'left' ? -16 : 16),
        elevation: Math.round(random.value * 16 - 8),
        radius: 0.8,
        height: 1.35,
      },
      emission: { source: 'sun', beat: beat - 2 },
      earlyMs: 180,
      lateMs: 240,
    });
    index++;
  ...
  if (!map.notes.length)
    throw new EngineError(
      'MUSIC_SILENT',
      'No audible signal found; choose another track or author a chart manually',
    );

...
The local Synth Riders study examined 25 underlying charts and subsequently inspected compiled spin-planning behavior. Useful principles were repeated gestures, returning rhythms with changed geometry, independent held and tapping hands, beat-timed turning gestures, authored light cues, song offsets, and difficulty changes beyond event count. StateBeats uses original policies, phrases and procedural content; no commercial code, charts or audio are included. No live comparative headset study has been completed.

## Implemented foundation

[Event Horizon — Master](EVENT_HORIZON.md) is the complete reference composition: bundled
original music, score-aligned expert choreography, musical turn phrases, independent rails and
...
- Versioned musical turn tracks, replaceable cue planning, analytic peak speed/acceleration and directional-travel budgets, recorded reasons and pure SDK/CLI/MCP inspection. Short gestures and sustained phrases share the same authoring contract.
...
4. **Extend facing tracks into an authoring view.** Beat-timed turn cues, recorded decisions and independent emitter/contact geometry are implemented in the SDK. Add a visual editor and preview of the player's view throughout a turn, including rear and overhead approaches. Keep source position, travel duration and facing independent. Full spherical content belongs in the engine; overhead choreography requires its own comfort and readability evidence. Add authored scene cues alongside continuous music features so a moving sun or black hole can emphasize selected musical moments.
